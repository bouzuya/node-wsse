...
const setUp = () => {
  ...
  return {
    ...
    token: new UsernameToken({
      created, nonce, password, sha1encoding, username
    }),
    username
  };
};
...
const tests: Test[] = [
  test(category + '#getCreated', fixture(setUp, tearDown, ({ created, token }) => {
    assert(token.getCreated() === created);
  })),
  test('#getNonce', fixture(setUp, tearDown, ({ nonce, token }) => {
    assert(token.getNonce() === nonce);
  })),
  test('#getNonceBase64', fixture(setUp, tearDown, () => {
    // do nothing
  })),
  test('#getPassword', fixture(setUp, tearDown, ({ password, token }) => {
    assert(token.getPassword() === password);
  })),
  test('#getPasswordDigest', fixture(setUp, tearDown, ({ digest, token }) => {
    assert(token.getPasswordDigest() === digest);
  })),
  test('#getUsername', fixture(setUp, tearDown, ({ token, username }) => {
    assert(token.getUsername() === username);
  })),
  test('#getWSSEHeader with no args', fixture(setUp, tearDown, ({ header, token }) => {
    assert(token.getWSSEHeader() === header);
  })),
  test('#getWSSEHeader with { nonceBase64: true }', fixture(setUp, tearDown, () => {
    // TODO
  })),
  test('#newToken', fixture(setUp, tearDown, ({ token }) => {
  ...
  test('#toString', fixture(setUp, tearDown, ({ header, token }) => {
    assert(token.toString() === header);
    assert(token + '' === header);
  })),
  test('#toString with { nonceBase64: true }', fixture(setUp, tearDown, () => {
    // do nothing
  }))
];
